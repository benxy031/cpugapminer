...
/* ── CRT (Chinese Remainder Theorem) sieve template ──
   Loaded from a file generated by tools/gen_crt.
   Replaces the small-prime sieve phase with a precomputed bitmap template
   that is tiled into the sieve bitmap via rotated memcpy.                  */
...
/* Load a CRT file and build the odd-only composite bitmap template.
...
static int load_crt_file(const char *path) {
    ...
    g_crt_period     = (size_t)(hdr.prim / 2);  /* odd-only period */
    ...
    for (uint64_t i = 0; i < hdr.ncand; i++) {
        ...
        if (v < 1 || !(v & 1)) continue; /* skip even / zero */
        size_t bit = (v - 1) / 2;
        if (bit < g_crt_period)
            g_crt_template[bit / 8] &= ~(1u << (bit & 7));
    }
    /* Clear any trailing bits beyond period (mark as composite so they
       don't sneak through during tiling of a partial final period). */
    ...
    log_msg("CRT: loaded %s  primes=%d (up to %llu)  primorial=%llu  "
...
static uint64_t* sieve_range(uint64_t L, uint64_t R, size_t *out_count,
    ...
    /* ── CRT fast-init path ──
       If a CRT template is loaded, tile it into the bitmap instead of
       memset(0) + small-prime sieve.  The template already marks composites
       for the first g_crt_n_primes primes, so the sieve loop starts at
       the first prime AFTER those covered by CRT.                         */
    int crt_skip_to = 0;   /* small_primes_cache index to start sieving from */
    if (g_crt_template && g_crt_primorial > 0) {
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        printf("      --crt-file FILE   load CRT sieve template (from gen_crt tool)\n");
